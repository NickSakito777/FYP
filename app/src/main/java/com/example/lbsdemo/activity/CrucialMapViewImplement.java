...
public class CrucialMapViewImplement extends AppCompatActivity implements SensorEventListener {
    ...
    private Location lastLocation; // 最后一次获取的位置信息
    private String currentTaskType = null;
    private int currentTaskId = -1; // <-- Add member variable to store task ID
    /*这是最重要的java Class
       实现功能如下：
    ...
    private void showPopupLayer(LatLng position, String title, String description, String markerId) {
        ...
        if (isCheckingIn) {
            ...
            CheckInButton.setOnClickListener(v -> {
                // 根据任务类型区分处理逻辑
                if (ActivitySelection.TASK_TYPE_AGENT.equals(currentTaskType)) {
                    // --- Start: Add Location Check for Agent Task Timer ---
                    // 检查位置是否匹配
                    if (currentFenceId != null && currentFenceId.equals(markerId)) {
                        // 位置匹配，可以开始计时
                        Log.d("TaskType", "特工任务打卡：位置匹配，开始计时流程");
                        // 设置正在打卡状态
                        SharedPreferences prefs = getSharedPreferences("checkin_status", MODE_PRIVATE);
                        SharedPreferences.Editor editor = prefs.edit();
                        editor.putBoolean("checking_in_" + markerId, true); // 标记正在打卡
                        editor.apply();
                        ...
                        // 显示带计时器的弹窗
                        showTimerView(markerId); // 这将启动计时器
                    } else {
                        // 位置不匹配，显示提示
                        Toast.makeText(this, "任务需要在指定位置 (" + markerId + ")，请前往指定地点", Toast.LENGTH_SHORT).show();
                        // 不执行后续操作，用户需要移动到正确位置
                    }
                    // --- End: Add Location Check for Agent Task Timer ---
                } else {
                    // 日常任务：直接调用原有的打卡逻辑
    ...
    private void finishTimer() {
        ...
        new AlertDialog.Builder(this)
            ...
            .setPositiveButton("确定", (dialog, which) -> {
                ...
                // --- Modify: Use stored currentTaskId instead of getIntent() ---
                // 从原始Intent中获取任务ID (Original, now incorrect)
                // int taskId = getIntent().getIntExtra("task_id", -1); 
                int taskId = this.currentTaskId; // Use the stored task ID
                // --- End Modify ---

                activitySelectionIntent.putExtra("task_id", taskId);
                Log.i("CrucialMapViewImplement", "[finishTimer] Using taskId: " + taskId);
                // 设置标志，保持正常的活动栈
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode == REQUEST_TASK_TIMER) {
            ...
            if (ActivitySelection.TASK_TYPE_AGENT.equals(currentTaskType)) {
                ...
                activitySelectionIntent.putExtra("task_id", taskId);
                Log.i("CrucialMapViewImplement", "[onActivityResult - Photo] Using taskId: " + taskId);
                // 设置标志，保持正常的活动栈
                activitySelectionIntent.addFlags(Intent.FLAG_ACTIVITY_REORDER_TO_FRONT);
                startActivity(activitySelectionIntent);
            ...
            // Read task details returned from ActivitySelection
            this.currentTaskId = data.getIntExtra("task_id", -1); // <-- Store the returned task ID
            Log.i("CrucialMapViewImplement", "[onActivityResult - Selection] Received and stored taskId: " + this.currentTaskId);

            // Clear existing markers before adding new ones
            mBaiduMap.clear(); 
            taskMarkers.clear(); // Also clear the marker map
            
            // Get task type returned from ActivitySelection
            currentTaskType = data.getStringExtra(ActivitySelection.EXTRA_TASK_TYPE);
            Log.d("CrucialMapViewImplement", "Received taskType: " + currentTaskType);
